...
const OfficeSuite: React.FC<OfficeSuiteProps> = ({ onShare }) => {
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            if (['summarizer', 'translator', 'content-plan', 'ai-pdf', 'formula', 'data-analysis'].includes(activeTool.id)) {
            ...
            {!activeTool ? (
                <>
                    <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                        <div>
                            <h2 className="text-3xl font-bold text-white">Office & Business</h2>
                            <p className="text-slate-400">Productivity tools for your entire workflow.</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 overflow-y-auto pb-20 custom-scrollbar">
                        {filteredTools.map(tool => (
                            <button key={tool.id} onClick={() => setActiveTool(tool)} className="bg-slate-900/50 hover:bg-slate-800 border border-slate-800 hover:border-cyan-500/50 p-4 rounded-xl flex flex-col items-center text-center transition group h-40 justify-center">
                    ...
                    <div className="flex-grow p-8 flex flex-col items-center justify-center overflow-y-auto">
                        {activeTool.id === 'sticky-notes' ? (
                            <div className="w-full h-full relative bg-slate-800/50 rounded-xl border-2 border-dashed border-slate-700 overflow-hidden">
                                ...
                                {notes.map(note => (
                                    <div key={note.id} className={`absolute w-40 h-40 p-4 shadow-xl rounded ${note.color} text-slate-900 cursor-move`} style={{top: note.y, left: note.x}} draggable onDragEnd={(e) => {
                                        const newNotes = notes.map(n => n.id === note.id ? {...n, x: e.clientX % 500, y: e.clientY % 400} : n); // simplified drag
                                        setNotes(newNotes);
                                    }}>
                            ...
                            <div className="w-full max-w-2xl space-y-6">
                                ...
                                {['Summarizer', 'Translate', 'Content', 'Formula', 'Quiz', 'Data', 'URL', 'Bio'].some(k => activeTool.name.includes(k) || activeTool.category.includes(k)) && (
                                    <textarea 
                                        value={input} 
                                        onChange={e => setInput(e.target.value)} 
                                        className="w-full bg-slate-950 border border-slate-700 rounded-lg p-4 text-white min-h-[150px]" 
                                        placeholder={activeTool.id === 'url-short' ? "Paste URL here..." : "Enter text, data, or topic..."}
                                    />
                                )}

                                <button onClick={handleProcess} disabled={loading} className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3 rounded-lg transition disabled:opacity-50">
